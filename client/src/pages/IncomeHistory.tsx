      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-end gap-2">
            <span className="font-semibold tabular-nums text-green-600" data-testid={`text-amount-${transaction.id}`}>
...
export default function IncomeHistory() {
  const { transactions, isLoading, deleteTransaction } = useData();
  ...
  const handleDelete = (transaction: Transaction) => {
    ...
    toast({
      title: "삭제 완료",
      description: `"${transaction.title}" 수입이 삭제되었습니다`,
    });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-4 pb-24 md:pb-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold" data-testid="text-income-history-title">
            수입 내역
          </h1>
          <p className="text-muted-foreground">
            총 {incomeTransactions.length}건 · {formatCurrency(totalIncome)}
          </p>
        </div>
      </div>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={(open) => !open && setDeleteConfirm(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              "{deleteConfirm?.title}" 수입을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.
            </DialogDescription>
          </DialogHeader>
